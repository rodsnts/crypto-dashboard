...
function CryptoTable({ currency, cryptoData }: CryptoTableProps) {
  ...
  return (
    <div className="container mx-auto p-4">
      {cryptoData ? (<div className="overflow-x-auto">
        <table className="w-full bg-white border border-gray-200 rounded-lg overflow-hidden">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Name</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Price</th>
              <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Market cap ({currency?.toUpperCase()})</th>
            ...
            {Object.keys(cryptoData).map((crypto) => (
              <tr key={crypto} className="hover:bg-gray-50">
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm font-medium text-gray-900">{crypto?.toUpperCase()}</div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right">
                  <div className={`text-sm flex items-center justify-end ${cryptoData[crypto][`${currency}_24h_change`] >= 0 ? 'text-green-600' : 'text-red-600'}`}>
